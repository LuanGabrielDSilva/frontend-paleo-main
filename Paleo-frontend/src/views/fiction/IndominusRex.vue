<template>
  <div class="museum-page" data-testid="indominus-rex-page">
    <!-- Grain overlay -->
    ...
    <header class="hero">
      <div class="hero__media">
        <img :src="image" alt="Indominus Rex" class="hero__img" />
        <div class="hero__fade"></div>
      </div>

      <div class="hero__content">
        <span class="kicker">Arquivos Confidenciais · InGen Bio-Engineering</span>
        <h1 class="title">
          <span class="title__pre">Espécime Híbrido N°001</span>
          <span class="title__main">Indominus rex</span>
          <span class="title__post">— "A Rainha Branca"</span>
        </h1>

        <p class="lede">
          Concebida nos laboratórios da <em>Hammond Creation Lab</em> em <strong>Isla Nublar</strong>,
          a Indominus rex foi a primeira teropoda híbrida bem-sucedida da InGen. Maior, mais inteligente
          e mais agressiva que qualquer carnívoro registrado, foi liberada por acidente em 2015,
          desencadeando o "Incidente Jurassic World".
        </p>

        <!-- AUDIO -->
        <div class="audio-plate" data-testid="indominus-audio">
          <div class="audio-plate__label">
            <span class="dot"></span>
            Registro Sonoro · Vocalização de Caça
          </div>
    ...
    <section class="ledger">
      ...
      <div class="ledger__grid">
        ...
        <div class="stat"><dt>Era de Concepção</dt><dd>2012 – 2014</dd></div>
        <div class="stat"><dt>Comprimento</dt><dd>13 – 15 m</dd></div>
        <div class="stat"><dt>Altura</dt><dd>~6 m (cabeça)</dd></div>
        <div class="stat"><dt>Massa estimada</dt><dd>8 – 9 toneladas</dd></div>
        <div class="stat"><dt>Velocidade</dt><dd>até 48 km/h</dd></div>
        <div class="stat"><dt>Dieta</dt><dd>Hipercarnívora</dd></div>
        <div class="stat status"><dt>Status</dt><dd>Eliminada · Lagoa Central</dd></div>
      </div>
    </section>
    ...
    <section class="genome">
      ...
      <p class="genome__intro">
        Genoma base de <em>Tyrannosaurus rex</em>, complementado por sequências de:
      </p>
    ...
    <section class="behavior">
      ...
      <div class="behavior__grid">
        <article>
          <h3>Cognição</h3>
          <p>
            Demonstrou capacidade de planejamento espacial, memória de longo prazo e
            aprendizado por tentativa. Sabia reconhecer câmeras térmicas e
            <strong>arrancou seu próprio implante rastreador</strong> após observar o
            sistema de monitoramento de seu paddock.
          </p>
        </article>
        <article>
          <h3>Camuflagem ativa</h3>
          <p>
            Cromatóforos derivados do choco (<em>Sepiidae</em>) permitiam alterar a pigmentação
            da pele em segundos, integrando-se à vegetação. Combinada à supressão térmica
            (DNA de perereca arbórea), tornava-a praticamente invisível ao infravermelho.
          </p>
        </article>
        <article>
          <h3>Caça por esporte</h3>
          <p>
            Diferente dos teropodas naturais, a I. rex matava sem intuito de alimentação.
            Os Apatossauros encontrados no Setor 04 apresentavam ferimentos profundos
            mas haviam sido <em>abandonados intactos</em> — comportamento sem precedentes.
          </p>
        </article>
      </div>
    </section>
    ...
    <section class="combat">
      ...
      <div class="combat__bars">
        <div v-for="s in stats" :key="s.label" class="bar">
          <div class="bar__top">
            <span>{{ s.label }}</span><span>{{ s.value }}/10</span>
          </div>
          <div class="bar__track">
            <div class="bar__fill" :style="{ width: (s.value * 10) + '%' }"></div>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="notes">
      ...
      <ol class="notes__list">
        <li>Únicas duas amostras geradas; a irmã foi devorada pela própria I. rex ainda em incubação.</li>
        <li>Solitária por natureza — não tolerava nem mesmo a presença humana de seus criadores.</li>
        <li>Comunicava-se com Velociraptores em frequências sub-sônicas, recrutando-os temporariamente como matilha.</li>
        <li>Resistente a calibres .50 BMG na região do tórax devido à blindagem dérmica reforçada.</li>
        <li>Eliminada no Setor Principal por <strong>Rexy (T. rex de 1993)</strong> em conjunto com Blue e o Mosassauro.</li>
      </ol>
    </section>
...
import image from '@/assets/fiction/indominus.jpg'
import roar from '@/assets/sounds/Sons e Rugidos da Indominus Rex - Jurassic World O Mundo dos Dinossauros! - Jurassic Fans (youtube).mp3'

const genome = [
  { name: 'Velociraptor', trait: 'agilidade, inteligência de matilha' },
  { name: 'Carnotaurus', trait: 'estrutura craniana e chifres atenuados' },
  { name: 'Giganotosaurus', trait: 'massa muscular e mordida potencializada' },
  { name: 'Majungasaurus', trait: 'resistência óssea' },
  { name: 'Rugops', trait: 'placa dérmica reforçada' },
  { name: 'Therizinosaurus', trait: 'garras dianteiras alongadas (≈30 cm)' },
  { name: 'Sepiidae (Choco)', trait: 'camuflagem cromática ativa' },
  { name: 'Hylidae (Perereca)', trait: 'supressão de assinatura térmica' }
]

const stats = [
  { label: 'Força bruta', value: 10 },
  { label: 'Velocidade', value: 7 },
  { label: 'Inteligência', value: 9 },
  { label: 'Furtividade', value: 9 },
...
.hero__media { position: relative; aspect-ratio: 4/5; border: 1px solid var(--rule);
...
.lede { font-size: 20px; line-height: 1.7; color: var(--ink); max-width: 56ch; margin: 0 0 36px; }
...
.audio-plate {
  ...
  background: linear-gradient(180deg, rgba(201,165,87,0.06), rgba(0,0,0,0.4));
  padding: 18px 20px;
  position: relative;
}
.audio-plate::before, .audio-plate::after {
  content: ""; position: absolute; width: 6px; height: 6px; background: var(--gold); border-radius: 50%;
}
.audio-plate::before { top: 8px; left: 8px; }
.audio-plate::after  { bottom: 8px; right: 8px; }
.audio-plate__label {
  font-family: 'Courier New', monospace; font-size: 11px; letter-spacing: 0.3em;
...
.section-title {
  ...
  color: var(--gold-bright); margin: 0 0 36px;
...
.section-title::after {
  content:""; flex:1; height: 1px; background: var(--rule); margin-left: 10px;
}
...
.stat {
  ...
  background: linear-gradient(180deg, rgba(201,165,87,0.03), transparent);
}
.stat dt { font-family: 'Courier New', monospace; font-size: 10px; letter-spacing: 0.3em;
...
.genome__list li {
  ...
  padding: 14px 0; border-bottom: 1px dashed var(--rule); gap: 20px;
}
...
.behavior__grid h3 { font-family: 'Cinzel', serif; color: var(--gold-bright); font-weight: 500;
  letter-spacing: 0.1em; text-transform: uppercase; font-size: 15px; margin: 0 0 12px; }
.behavior__grid p { font-size: 17px; line-height: 1.65; color: var(--ink); margin: 0; }

/* Combat */
.combat__bars { display: grid; grid-template-columns: repeat(2, 1fr); gap: 28px 60px; }
.bar__top { display: flex; justify-content: space-between; font-family: 'Courier New', monospace;
  font-size: 12px; letter-spacing: 0.2em; text-transform: uppercase; color: var(--ink-soft); margin-bottom: 8px; }
...
.notes__list li {
  counter-increment: n; position: relative; padding: 18px 0 18px 56px;
  border-bottom: 1px solid var(--rule); font-size: 18px; line-height: 1.6;
}
...
@media (max-width: 560px) {
  .ledger__grid, .combat__bars, .genome__list { grid-template-columns: 1fr; }
}
</style>
